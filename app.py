...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///Wallet-manage.db'
...
def add_expense():
    ...
    if not (title  and amount and category_id):
        return jsonify({'message': 'All fields are required'}), 400

    new_expense = Expense(title=title, amount=amount, category_id=category_id, user_id=get_jwt_identity())
